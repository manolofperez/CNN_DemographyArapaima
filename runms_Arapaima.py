...
for i in range(Priorsize):
	...
	GrowthAM = -(1/coalAM_ArTO_DivTime-coalCod_DivTime)*math.log((1/GrowthRatio)/(1/FoundedSizeRatio))
	GrowthTA = -(1/coalAM_ArTO_DivTime)*math.log((1/GrowthRatio)/(1/FoundedSizeRatio))
	## nDNA ms's command
	com=subprocess.Popen("./ms %d 300 -s 1 -t %f -I 3 %d %d %d -g 3 %f -ej %f 2 1 -eg %f 1 %f -en %f 1 %f -en %f 3 %f -ej %f 3 1" % (nDNANsam, Theta, nDNAPop1, nDNAPop2, nDNAPop3, GrowthTA, coalCod_DivTime, coalCod_DivTime, GrowthAM, coalAM_ArTO_DivTime, FoundedSizeRatio, coalAM_ArTO_DivTime, FoundedSizeRatio, coalAM_ArTO_DivTime), shell=True, stdout=subprocess.PIPE).stdout
 	output = com.read().splitlines()
...
for i in range(Priorsize):
	...
	output = com.read().splitlines()
	simModel4.append(sort_min_diff(np.array(ms2nparray(output)).swapaxes(0,1).reshape(nDNANsam,-1)))
...
simModel4=simModel4.swapaxes(1,2)
np.savez_compressed('simModel4.npz', simModel4=simModel4)
